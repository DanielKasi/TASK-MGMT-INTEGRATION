...
import { ChatMessageItem } from "./chat-task-message-item";
import { ConfirmationDialog } from "@/components/confirmation-dialog";
import { AddUsersToChatDialog } from "@/components/common/dialogs/add-users-to-chat-dialog";
import { ManageParticipantsDialog } from "@/components/common/dialogs/manage-task-chat-participants-dialog";
import {
  ITaskChatMessage,
...
import {
  TASK_CHAT_API,
  TASK_CHAT_DISCUSSION_PARTICIPANT_API,
...
export function TaskChatThread({
  ...
  const [messages, setMessages] = useState<ITaskChatMessage[]>([]);
  ...
  const [currentUserParticipant, setCurrentUserParticipant] =
    useState<IChatDiscussionParticipants | null>(null);
  ...
  const isUserAtBottom = useCallback(() => {
    ...
    return scrollHeight - scrollTop - clientHeight < 50; // Consider "at bottom" if within 50px
  }, []);

  const fetchParticipants = useCallback(async () => {
    try {
      const res =
        await TASK_CHAT_DISCUSSION_PARTICIPANT_API.getPaginatedTaskDiscussionParticipants(
          {
            page: 1,
          }
        );
  ...
  const fetchFirstPage = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await TASK_CHAT_API.getPaginatedMessages({
        task: taskId,
        page: 1,
      });
  ...
  const fetchNextPage = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await TASK_CHAT_API.getMessagesFromUrl({ url: nextUrl });
  ...
  const handleSendMessage = async (content: string) => {
    ...
    setSending(true);
    try {
      const data: ITaskChatMessageFormData = {
        content,
        task: taskId,
      };

      const newMessage = await TASK_CHAT_API.create({ data });
  ...
  const handleEditMessage = async (messageId: number, newContent: string) => {
    try {
      const updatedMessage = await TASK_CHAT_API.update({
        messageId,
  ...
  const handleDeleteMessage = async () => {
    ...
    setIsDeleting(true);
    try {
      await TASK_CHAT_API.delete({ messageId: messageToDelete });
