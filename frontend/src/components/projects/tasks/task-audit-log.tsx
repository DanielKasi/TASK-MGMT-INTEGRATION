...
import { CONTENT_TYPES, type IAuditTaskLogs } from "@/types/project.type";
...
export function TaskAuditTrail({ taskId, className }: TaskAuditTrailProps) {
  const [logs, setTaskLogs] = useState<IAuditTaskLogs[]>([]);
  ...
  const fetchAuditTaskLogs = useCallback(async () => {
    if (!taskId) return;

    setLoading(true);
    try {
      const res = await AUDIT_LOG_API.getPaginatedTaskAuditLogs({
        task_id: taskId,
        page: 1,
        object_id: taskId,
      ...
      setTaskLogs(res.results.reverse());
      setNextUrl(res.next);
      setHasMore(!!res.next);

      setTimeout(scrollToBottom, 100);
    } catch (error) {
      showErrorToast({ error, defaultMessage: "Failed to fetch task logs" });
      setHasMore(false);
    } finally {
      setLoading(false);
    }
  ...
  const fetchNextPage = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await AUDIT_LOG_API.getAuditTaskLogsFromUrl({ url: nextUrl });

      setTaskLogs((prev) => [...res.results.reverse(), ...prev]);
      setNextUrl(res.next);
      setHasMore(!!res.next);
    } catch (error) {
      showErrorToast({
        error,
        defaultMessage: "Failed to load more task logs",
      });
      setHasMore(false);
    } finally {
      setLoading(false);
  ...
  return (
    <div
      className={cn(
        "max-h-[calc(100vh-200px)] overflow-y-auto space-y-6 pr-4",
        className
      )}
    >
      {logs.map((log, index) => {
