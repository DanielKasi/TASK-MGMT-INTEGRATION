...
export function ManageParticipantsDialog({
  ...
  const handleToggleMessagePermission = async (
    ...
    try {
      const updatedParticipant =
        await TASK_PROJECT_DISCUSSION_PARTICIPANT_API.update({
          id: participant.id,
          data: { can_send: !participant.can_send },
        });
      ...
      showErrorToast({ error, defaultMessage: "Failed to update participant" });
    } finally {
  ...
  const handleDeleteParticipant = async () => {
    ...
    setIsDeleting(true);
    try {
      await TASK_PROJECT_DISCUSSION_PARTICIPANT_API.delete({
        id: participantToDelete.id,
      });

      toast.success("Participant removed from project chat");
      onParticipantsChanged?.();
      await new Promise((resolve) => setTimeout(resolve, 100));
    } catch (error) {
      showErrorToast({ error, defaultMessage: "Failed to remove participant" });
    } finally {
    ...
    <>
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] flex flex-col p-0">
          <DialogHeader className="px-6 pt-6 pb-4 border-b">
            <DialogTitle className="flex items-center gap-2">
              ...
              Manage Project Chat Participants
            </DialogTitle>
            <DialogDescription>
              View and manage who can participate in this project discussion
            </DialogDescription>
          </DialogHeader>

          <ScrollArea className="flex-1 px-6 max-h-[60vh]">
            <div className="space-y-6 py-4">
              {/* Project Assignees Section */}
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <Shield className="h-4 w-4 text-primary" />
                  <h3 className="text-sm font-semibold">
                    Project Assignees ({projectAssignees.length})
                  </h3>
                </div>
      ...
      {participantToDelete && (
        <ConfirmationDialog
          ...
          description="Are you sure you want to remove this participant from the project chat? They will no longer be able to view or send messages."
