...
import { TASK_CHAT_DISCUSSION_PARTICIPANT_API } from "@/lib/utils.chats";
import UserSearchableSelect from "@/components/selects/user-searchable-select";
import { IChatDiscussionParticipantsFormData } from "@/types/project.type";
...
export function AddUsersToChatDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const promises = selectedUserIds.map((userId) => {
        const data: Partial<IChatDiscussionParticipantsFormData> = {
          task: taskId,
          user: Number(userId),
          can_send: !isViewerOnly,
        };
        return TASK_CHAT_DISCUSSION_PARTICIPANT_API.create({ data });
      });

      await Promise.all(promises);

      const roleText = isViewerOnly ? "viewer(s)" : "participant(s)";
      toast.success(`${selectedUserIds.length} ${roleText} added to chat`);
      onUsersAdded?.();
      onOpenChange(false);
    } catch (error) {
      showErrorToast({ error, defaultMessage: "Failed to add users to chat" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const excludedUserIds = [
    ...new Set([...taskAssigneeIds, ...existingParticipantIds]),
  ];
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Add users to this task discussion. Task assignees are already
        ...
        <div className="space-y-4 py-4">
          <Alert>
            ...
            <AlertDescription className="text-xs">
              Users assigned to this task are automatically included in the chat
