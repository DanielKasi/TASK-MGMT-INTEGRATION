...
export default function CompletedTasksPage() {
  const [tasks, setTasks] = useState<ICompletedTasks[]>([]);
  ...
  const [taskToDelete, setTaskToDelete] =
    useState<ICompletedTasks | null>(null);
  ...
  const fetchFirstPage = useCallback(async () => {
    ...
    try {
      ...
      const res =
        await TASKS_STATUSES_API.getPaginatedCompletedTasks(
          params
        );
  ...
  const fetchNextPage = useCallback(async () => {
    ...
    try {
      const res =
        await TASKS_STATUSES_API.getPaginatedCompletedTasksFromUrl(
          { url: nextUrl }
        );
            ...
            {tasks.map((task) => (
              ...
              >
                ...
                <div className="flex gap-2 mt-2 sm:mt-3">
                  <Badge
                    style={{
                      backgroundColor: task.applied_task_status
                        ?.color_code
