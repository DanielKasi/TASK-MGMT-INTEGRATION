...
import { ManageParticipantsDialog } from "@/components/common/dialogs/manage-project-task-chat-participants-dialog";
import type { IProjectChatDiscussionParticipants } from "@/types/project.type";
import {
  TASK_PROJECT_DISCUSSION_PARTICIPANT_API,
  PROJECT_CHAT_API,
} from "@/lib/utils.chats";
import { ChatInput } from "./chat-input";
import { ChatMessageItem } from "./chat-project-message-item";
...
interface ProjectChatThreadProps {
  projectId: number;
  className?: string;
...
export function ProjectChatThread({
  ...
  const fetchParticipants = useCallback(async () => {
    try {
      const res =
        await TASK_PROJECT_DISCUSSION_PARTICIPANT_API.getPaginatedProjectDiscussionParticipants(
          {
            project: projectId,
            page: 1,
          }
        );
      const projectParticipants = res.results.filter(
        (p) => p.project === projectId
      );
      setParticipants(projectParticipants);

      const userParticipant = projectParticipants.find(
  ...
  const fetchFirstPage = useCallback(async () => {
    if (!currentInstitution || !projectId) return;

    setLoading(true);
    try {
      const res = await PROJECT_CHAT_API.getPaginatedMessages({
        project: projectId,
        page: 1,
      });
  ...
  }, [currentInstitution, projectId, scrollToBottom]);
  ...
  const fetchNextPage = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await PROJECT_CHAT_API.getMessagesFromUrl({ url: nextUrl });
  ...
  const handleSendMessage = async (content: string) => {
    ...
    setSending(true);
    try {
      const data: IProjectChatMessageFormData = {
        content,
        project: projectId,
      };

      const newMessage = await PROJECT_CHAT_API.create({ data });
  ...
  const handleEditMessage = async (messageId: number, newContent: string) => {
    try {
      const updatedMessage = await PROJECT_CHAT_API.update({
        messageId,
  ...
  const handleDeleteMessage = async () => {
    ...
    setIsDeleting(true);
    try {
      await PROJECT_CHAT_API.delete({ messageId: messageToDelete });
  ...
  // Check if current user is a project assignee
  const isProjectAssignee = taskAssignees.some(
    (assignee) => assignee.id === currentUser?.id
  );

  // Determine if user can send messages
  const canSendMessages =
    isProjectAssignee || (currentUserParticipant?.can_send ?? false);

  // Get participant user IDs
  const participantUserIds = participants
    .map((p) => (p as any).user?.id)
    .filter(Boolean);
  const projectAssigneeIds = taskAssignees.map((a) => a.id);
  const totalParticipants = projectAssigneeIds.length + participants.length;
  ...
  return (
    ...
    >
      ...
      <AddUsersToChatDialog
        ...
        taskId={projectId}
        taskAssigneeIds={projectAssigneeIds}
      ...
      <ManageParticipantsDialog
        ...
        projectAssignees={taskAssignees}
