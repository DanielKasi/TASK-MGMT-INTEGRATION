...
interface TaskPriorityDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: () => void;
  initialData?: ITaskPriority | null;
}
...
export function TaskPriorityDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const priorityData: Partial<ITaskPriorityFormData> = {
        name: priorityName,
      ...
      <DialogContent className="max-w-[90vw] sm:max-w-xl md:max-w-2xl lg:max-w-3xl rounded-xl p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">
            {initialData ? "Edit Task Priority" : "Create Task Priority"}
          </DialogTitle>
        </DialogHeader>
