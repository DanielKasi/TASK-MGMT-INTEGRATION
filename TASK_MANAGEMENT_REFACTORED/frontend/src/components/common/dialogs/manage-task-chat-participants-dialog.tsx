...
import { IChatDiscussionParticipants } from "@/types/project.type";
...
interface ManageParticipantsDialogProps {
  ...
  taskId: number;
  participants: IChatDiscussionParticipants[];
...
export function ManageParticipantsDialog({
  ...
  const handleToggleMessagePermission = async (
    participant: IChatDiscussionParticipants
  ) => {
    setUpdatingIds((prev) => [...prev, participant.id]);
    try {
      const updatedParticipant =
        await TASK_CHAT_DISCUSSION_PARTICIPANT_API.update({
          id: participant.id,
          data: {
  ...
  const handleDeleteParticipant = async () => {
    ...
    setIsDeleting(true);
    try {
      await TASK_CHAT_DISCUSSION_PARTICIPANT_API.delete({
