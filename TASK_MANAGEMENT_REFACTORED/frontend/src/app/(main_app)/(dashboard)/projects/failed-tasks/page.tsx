...
import { IProjectFailedTasks } from "@/types/project.type";
import {
  showErrorToast,
  PROJECTS_TASKS_API,
...
import { IProjectTaskPriority } from "@/types/project.type";
import { hasPermission } from "@/lib/helpers";
import RichTextDisplay from "@/components/common/rich-text-display";
import { useParams } from "next/navigation";
import { PROJECT_TASKS_STATUSES_API } from "@/lib/utils.chats";
...
export default function FailedTasksPage() {
  ...
  const [tasks, setTasks] = useState<IProjectFailedTasks[]>([]);
  ...
  const [taskToDelete, setTaskToDelete] = useState<IProjectFailedTasks | null>(
    null
  );
  const [isDeleting, setIsDeleting] = useState(false);
  const [isAddTaskOpen, setIsAddTaskOpen] = useState(false);
  const [selectedTask, setSelectedTask] = useState<IProjectFailedTasks | null>(
    null
  );
  ...
  const fetchFirstPage = useCallback(async () => {
    ...
    try {
      ...
      const res =
        await PROJECT_TASKS_STATUSES_API.getPaginatedProjectFailedTasks(params);
  ...
  const fetchNextPage = useCallback(async () => {
    ...
    try {
      const res =
        await PROJECT_TASKS_STATUSES_API.getPaginatedProjectFailedTasksFromUrl({
          url: nextUrl,
        });
            ...
            {tasks.map((task) => (
              ...
              >
                            ...
                            href={
                              task.project
                                ? typeof task.project === "number"
                                  ? `/projects/${task.project}/tasks/${task.id}`
                                  : `/projects/${task.project.id}/tasks/${task.id}`
                                : `/task-mgt/task/${task.id}`
                            }
                          >
                ...
                <div className="flex gap-2 mt-2 sm:mt-3">
                  <Badge
                    style={{
                      backgroundColor: task.applied_project_task_status
                        ?.color_code
                        ? `${task.applied_project_task_status.color_code}30`
                        : `rgba(120, 120, 120, 0.3)`,
                      color: task.applied_project_task_status?.color_code
                        ? task.applied_project_task_status.color_code
                        : `rgba(120, 120, 120, 1)`,
                      border: `1.5px solid ${task.applied_project_task_status?.color_code ? task.applied_project_task_status.color_code : "rgba(120, 120, 120, 1)"}`,
                    }}
                    className="capitalize text-xs sm:text-sm"
                  >
                    {task.applied_project_task_status?.name || "No Status"}
                  </Badge>
                  <Badge
                    style={{
